...
The cross product is only defined for 3D vectors. However, in 2D we can compute a useful **scalar version**: $a_x b_y - a_y b_x$, which is the z-component of the cross product when treating 2D vectors as 3D with $z = 0$. This scalar gives the signed area of the parallelogram and the direction of rotation (positive = counterclockwise). It's commonly used in 2D robotics for torque and left/right-of-line tests.
</Callout>
...
The torque is 3 N·m around the z-axis. By the right-hand rule, positive z-torque means **counterclockwise** rotation when viewed from above — this is the standard convention in robotics.
</Example>
...
5. **NOT associative**: $\vec{a} \times (\vec{b} \times \vec{c}) \neq (\vec{a} \times \vec{b}) \times \vec{c}$ in general — unlike vector addition, you cannot regroup cross products. This matters in robotics dynamics.
