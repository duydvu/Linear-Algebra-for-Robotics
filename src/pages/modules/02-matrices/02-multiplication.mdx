...
In words: **Take row *i* from *A*, column *j* from *B*, multiply corresponding elements, and sum them.** Notice that each element $c_{ij}$ is the **dot product** of row *i* of *A* with column *j* of *B* — the same dot product from Module 1!
...
3. If $A = \begin{bmatrix}0 & -1\\1 & 0\end{bmatrix}$ (90° rotation) and $B = \begin{bmatrix}2 & 0\\0 & 1\end{bmatrix}$ (stretch x by 2), compute *AB* and *BA*. Are they equal?
...
3. $AB = \begin{bmatrix}0&-1\\2&0\end{bmatrix}$, $BA = \begin{bmatrix}0&-2\\1&0\end{bmatrix}$. They are **not** equal — *AB* stretches x first then rotates, while *BA* rotates first then stretches x. Non-uniform scaling does not commute with rotation because the direction of stretching matters.
